...
-- select count(*), tipo_transporte, parcial, dia from dataset_viajes_sube group by 2, 3, 4 having count(*) > 1;--

create or replace temp view vo_dataset_viajes_sube_deduplicated as
select
	tipo_transporte,
	dia,
	parcial,
...
create or replace temp view vo_personas_por_dia as
select
		dia,
		sum(cantidad) as total_personas
from vo_dataset_viajes_sube_deduplicated
group by 1;

create or replace temp view vo_impacto_gente as
select
	a.fecha,
	a.indice_calidad_aire,
	b.total_personas,
	round(cast((indice_calidad_aire * total_personas/100000) as numeric), 2) as impacto_poblacion
from vo_calidad_aire_agrupada_indice a
inner join vo_personas_por_dia b
	on
		a.fecha = b.dia and
		a.pm10_measure_flag;

select * from vo_impacto_gente order by indice_calidad_aire asc, impacto_poblacion asc limit 10